...
const vlookupWithDateOrEmpty = ({sheet, column, targetDate} = {}) => {
  ...
  const resultRowIndex = findRowIndex == -1 ? findFirstEmptyRowIndexInColumn({sheet, column}) : findRowIndex;
...
const findFirstEmptyRowIndexInColumn = ({ sheet, column }) => {
  ...
  const columnIndex = column.toUpperCase().charCodeAt(0) - 'A'.charCodeAt(0);
